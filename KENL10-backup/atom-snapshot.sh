...
create_snapshot() {
    ...
    if [ -d "$HOME/kenl/KENL2-gaming/play-cards" ]; then
        cp -r "$HOME/kenl/KENL2-gaming/play-cards" "$snapshot_dir/play-cards"
...
restore_snapshot() {
    ...
    if [ -d "$snapshot_dir/play-cards" ]; then
        mkdir -p "$HOME/kenl/KENL2-gaming/play-cards"
        cp -r "$snapshot_dir/play-cards/"* "$HOME/kenl/KENL2-gaming/play-cards/" 2>/dev/null || true
