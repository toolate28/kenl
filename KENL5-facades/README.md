...
KENL5 provides **visual identity and context switching** for all KENL modules. Each KENL context gets its own:
...
```
⚛️  KENL1  Framework    (Purple)  - ATOM+SAGE+OWI Core
🎮  KENL2  Gaming       (Red)     - Bazzite Gaming (GWI)
💻  KENL3  Development  (Blue)    - Bazzite-DX Development
📊  KENL4  Monitoring   (Green)   - Monitoring & Observability
🎨  KENL5  Facades      (Yellow)  - Theming & Customization
```
...
When working across multiple KENL modules, it's easy to forget which context you're in:
...
With KENL5 facades, each context is **immediately obvious**:

```bash
🎮  KENL2 user@bazzite:~/game-configs$
# ✅ Clear: You're in gaming context

💻  KENL3 user@bazzite:~/code$
...
# You're now in KENL1 (default) context:
...
2. Adds KENL integration to `~/.bashrc` (if not present)
...
# Output: Current context: KENL2-gaming
```
...
Available KENL contexts:

  1) KENL1-framework  ⚛️  Purple  - ATOM+SAGE+OWI Core
  2) KENL2-gaming     🎮  Red     - Bazzite Gaming (GWI)
  3) KENL3-dev        💻  Blue    - Bazzite-DX Development
  4) KENL4-monitoring 📊  Green   - Monitoring & Observability
  5) KENL5-facades    🎨  Yellow  - Facades & Theming
```
...
### KENL1: Framework (Purple ⚛️)

**Prompt:**
```bash
⚛️  KENL1 user@hostname:~/kenl/KENL1-framework$
```
...
**Prompt:**
```bash
🎮  KENL2 user@hostname:~/kenl/KENL2-gaming$
```
...
### KENL3: Development (Blue 💻)

**Prompt:**
```bash
💻  KENL3 user@hostname:~/kenl/KENL3-dev$
```
...
### KENL4: Monitoring (Green 📊)

**Prompt:**
```bash
📊  KENL4 user@hostname:~/kenl/KENL4-monitoring$
```
...
### KENL5: Facades (Yellow 🎨)

**Prompt:**
```bash
🎨  KENL5 user@hostname:~/kenl/KENL5-facades$
```
...
- KENL5 **respects this** by only modifying user space
...
```bash
# Host system (Bazzite)
⚛️  KENL1 user@bazzite:~$ distrobox enter ubuntu-dev

# Inside distrobox (inherits context)
⚛️  KENL1 user@ubuntu-dev:~$

# Switch context from host
./switch-kenl.sh dev

# Distrobox picks up new context
💻  KENL3 user@ubuntu-dev:~$
```
...
Create your own KENL context prompt:
...
export KENL_CONTEXT="custom"
export KENL_COLOR="red"
export KENL_ICON="🔥"

echo "Custom KENL context loaded!"
```
...
KENL5 can detect which KENL directory you're in:
...
claude: "What KENL context am I in?"
...
```bash
🎮  KENL2 user@local:~$ ssh remote-server
🎮  KENL2 user@remote:~$  # Context preserved!
```
...
**Problem**: Context goes back to KENL1 after reboot
...
⚛️  KENL1 user@bazzite:~$ atom STATUS "Morning system check"

# Afternoon: Gaming session
./switch-kenl.sh gaming
🎮  KENL2 user@bazzite:~$ cd play-cards && ./validate-playcard.sh elden-ring

# Evening: Development
./switch-kenl.sh dev
💻  KENL3 user@bazzite:~$ distrobox enter dev-env
💻  KENL3 user@dev-env:~$ code .

# Night: Check monitoring
./switch-kenl.sh monitoring
📊  KENL4 user@bazzite:~$ logdy
```
