# KENL2: Gaming - Play Cards & Proton Optimization
...
sequenceDiagram
    ...
    KENL2->>ProtonDB: Query compatibility
    ProtonDB-->>KENL2: Platinum rating (9.2/10)
    KENL2->>Steam: Check installed Proton versions
    Steam-->>KENL2: Proton 8.0, Experimental
    KENL2->>User: Recommendation: Install GE-Proton 9-20

    Note over KENL2,ProtonDB: ATOM-RESEARCH-20251110-001 created
```
...
| Lost after reinstall | KENL10 backup includes Play Card |
...
sequenceDiagram
    ...
    KENL2->>KENL8: Encrypt with GPG
    KENL8-->>KENL2: halo-infinite.yaml.gpg
    KENL2->>Matrix: Send encrypted file + public key
    Matrix->>Friend: Message received
    Friend->>Matrix: Download file
    Friend->>KENL8: Decrypt with GPG
    KENL8-->>Friend: halo-infinite.yaml (plaintext)
    Friend->>KENL2: Apply Play Card
    KENL2-->>Friend: ✅ Same config, same performance
```
...
```bash
# Install Proton GE via KENL0 (privileged)
kenl-switch 0
...
```bash
# Step 1: Research
🎮 KENL2 $ research-game.sh "Starfield"
...
🎮 KENL2 $ create-playcard.sh "Starfield"
...
🎮 KENL2 $ kenl-switch 10
💾 KENL10 $ snapshot-create "starfield-working-config"

✅ Snapshot includes Play Card

# Step 5: Share with friend
🎮 KENL2 $ kenl-switch 8
🔐 KENL8 $ encrypt-file play-cards/starfield.yaml
🎮 KENL2 $ matrix-send starfield.yaml.gpg @friend:matrix.org

Friend applies:
🎮 KENL2 $ apply-playcard.sh starfield.yaml
...
| Traditional Approach | KENL2 Approach |
|---------------------|----------------|
