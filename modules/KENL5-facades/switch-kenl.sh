...
export PATH="\$KENL_ROOT/modules/KENL1-framework/atom-sage-framework:\$PATH"
EOF

    echo "✅ Activated KENL$context context"
    echo ""
...
install_systemwide() {
    ...
    if command -v rpm-ostree &> /dev/null; then
        echo "✅ Detected rpm-ostree system (Bazzite)"
        echo ""
    ...
    if ! grep -q "kenl_profile" "$HOME/.bashrc" 2>/dev/null; then
        ...
        echo "✅ Added KENL integration to ~/.bashrc"
    else
        echo "✅ KENL integration already in ~/.bashrc"
    fi
