...
KENL8-iwi (Installing With Intent) is a standalone installation framework for Bazzite with cryptographic resource verification. It captures installation decisions, validates resources against trusted checksums, and ensures reproducible system setups through ATOM-tagged audit trails.
...
```bash
# KENL8-iwi operates from repository - no installation needed
...
ls -la ~/kenl/modules/KENL8-iwi/

# Verify executables are executable
test -x ~/kenl/modules/KENL8-iwi/bin/iwi-verify-resources && \
...
```bash
# KENL8-iwi has no system installation
...
```bash
# Update KENL8-iwi with kenl repository
cd ~/kenl
git pull origin main

# Verify resources after update
cd modules/KENL8-iwi
./bin/iwi-verify-resources
```
...
- **Module:** KENL8-iwi (Installing With Intent)
